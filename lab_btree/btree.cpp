...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    DataPair mid = *mid_elem_itr;
    parent->elements.insert(elem_itr, mid);
    parent->children.insert(child_itr, new_right);
    new_right->elements.assign(mid_elem_itr + 1, child->elements.end());
    if(!new_right->is_leaf)new_right->children.assign(mid_child_itr, child->children.end());
    new_left->elements.assign(child->elements.begin(), mid_elem_itr);
    if(!new_left->is_leaf)new_left->children.assign(child->children.begin(), mid_child_itr);
}
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    if(!subroot->elements.empty() && first_larger_idx < subroot->elements.size() && subroot->elements[first_larger_idx] == pair) return;
    else if(subroot->is_leaf) subroot->elements.insert(subroot->elements.begin() + first_larger_idx, pair);
    else {
